    ...
    try {
      ...
      for (var doc in servicesQuery.docs) {
        try {
            ...
            if (!providersData.containsKey(providerId)) {
              providersData[providerId] = {
                ...
                'providerName': (data['providerName'] ?? 'مزود خدمة')
                    .toString(),
                'location': (data['location'] ?? 'غير محدد').toString(),
                'itemsCount': 1,
          ...
          print('❌ خطأ في معالجة العنصر: $e');
          continue; // تجاهل هذا العنصر والانتقال للتالي
        }
      }
        ...
        builder: (context, snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final providerName = (provider['providerName'] ?? 'مزود خدمة')
                  .toString();
              final location = (provider['location'] ?? 'غير محدد').toString();
              final itemsCount = provider['itemsCount'] as int? ?? 0;
              ...
              return Card(
                ...
                child: InkWell(
                  borderRadius: BorderRadius.circular(16),
                  onTap: () {
                    // التحقق من أن providerId ليس فارغاً قبل الانتقال
                    if (providerId.isNotEmpty) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(
                          content: Text('خطأ: معرف المزود غير صحيح'),
        ...
        builder: (context, snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final name = (data['name'] ?? 'غير محدد').toString();
              final details = (data['details'] ?? '').toString();
              final price = (data['price'] ?? 'غير محدد').toString();
              final imageUrls = data['imageUrls'] as List<dynamic>? ?? [];
              final location = (data['location'] ?? 'غير محدد').toString();
                          ...
                          children: [
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                ElevatedButton(
                                  onPressed: () {
                                    // التنقل إلى صفحة تفاصيل العنصر للحجز
                                    final itemData = data;
                                    itemData['id'] = item.id;
                                    Navigator.push(
                                      context,
